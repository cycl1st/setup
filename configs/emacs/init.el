...
(use-package startup
  :no-require
  :custom
  (user-mail-address "r4cer.cyclist@gmail.com")
  (user-full-name "Haido"))
...
(when window-system
  (use-package modus-themes
    ...
    :config
    (set-face-attribute 'default nil :font "JetBrains Mono" :height
...
(use-package project
  :defer t
  :config
  (defun project-local-setup (&rest _args)
    (message "Project script debug log. Rest args %s" _args)
    (let* ((pr (project-current))
           (root (when pr (project-root pr)))
           (local-init (expand-file-name ".emacs-project.el" root)))
      (message "pr: %s, root: %s, local init: %s" pr root local-init)
      (when (file-exists-p local-init)
        (message "[DEBUG] loading %s" local-init)
        (load-file local-init))))
  (defun project--ignore-elpa-dirs (orig-fn dir)
    (unless (string-match-p "/elpa/" dir)
      (funcall orig-fn dir)))
  (advice-add 'project-try-vc :around #'project--ignore-elpa-dirs)
  (advice-add 'project-switch-project :after #'project-local-setup))
